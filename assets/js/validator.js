...
const pathIsEn=location.pathname.startsWith('/en'),pathIsTr=location.pathname.startsWith('/tr');let lang='tr';
...
function render(data){
  ...
  try{
    sessionStorage.setItem('ea_scan_'+data.domain, JSON.stringify(data));
    sessionStorage.setItem('ea_scan_latest', JSON.stringify(data));
  }catch(e){}
  let entReportBtn=document.getElementById('btnViewEnterpriseReport');
  if(!entReportBtn){
    entReportBtn=document.createElement('a');
    entReportBtn.id='btnViewEnterpriseReport';
    entReportBtn.className='btn-pdf-export btn-enterprise-report';
    entReportBtn.style.cssText='background:linear-gradient(135deg,#0066ff 0%,#0284c7 100%);border-color:#0066ff;color:#ffffff;text-decoration:none;display:inline-flex;align-items:center;gap:6px;font-weight:700;';
    const actionsWrap=document.querySelector('.result-head-actions');
    if(actionsWrap)actionsWrap.insertBefore(entReportBtn,actionsWrap.firstChild);
  }
  ...
  const totalRawInstances = rawFindings.length;
  const findingsMap = new Map();
  rawFindings.forEach(f => {
    const key = f.id || f.title;
    if (!findingsMap.has(key)) {
      const item = { ...f, affectedUrls: [] };
      if (f.url) item.affectedUrls.push(f.url);
      findingsMap.set(key, item);
    } else {
      const existing = findingsMap.get(key);
      if (f.url && !existing.affectedUrls.includes(f.url)) {
        existing.affectedUrls.push(f.url);
      }
    }
  });
  data.findings = Array.from(findingsMap.values());
  const counts = { all: data.findings.length, rawTotal: totalRawInstances, critical: 0, high: 0, medium: 0, low: 0 };
  data.findings.forEach(f => {
    const s = String(f.severity || '').toLowerCase().trim();
    if (s === 'critical' || s === 'crit' || s === 'danger' || s === 'error') counts.critical++;
    else if (s === 'high' || s === 'p0' || s === 'warn-high') counts.high++;
...
const DAG_STEPS=[
  ...
  },
  {
    ...
    nodeType: 'n8n-nodes-base.httpRequest',
    badge: 'PREFERRED',
    desc: isTr?'Google Search 10 Eylül 2026 resmî teknik dokümanına göre publisher.js ve Preferred Sources entegrasyonunu doğrular (AI Mode ve AI Overviews).':'Probes publisher.js SDK and Preferred Sources markup against official Google Search Sept 10, 2026 standard (AI Mode & AI Overviews).',
...
data.findings.forEach(f=>{const title=lang==='tr'?(f.titleTr||f.titleEn):(f.titleEn||f.titleTr),impact=lang==='tr'?(f.impactTr||f.impactEn):(f.impactEn||f.impactTr),c=(conf[f.confidence]||{tr:f.confidence,en:f.confidence})[lang];
...
filterBar.querySelectorAll('.filter-btn').forEach(btn=>{
  btn.addEventListener('click',e=>{
    ...
    btn.classList.add('active');
    const rawTarget=btn.dataset.filter||'all';
    const target=rawTarget.toLowerCase().trim();
    let visibleCount=0;
    document.querySelectorAll('#findingsList .finding').forEach(item=>{
      const itemSev=(item.dataset.severity||'').toLowerCase().trim();
      let match=false;
      if(target==='all'){
        match=true;
      }else if(target==='critical'){
        match=(itemSev==='critical'||itemSev==='crit'||itemSev==='danger'||itemSev==='error');
      }else if(target==='high'){
        match=(itemSev==='high'||itemSev==='p0'||itemSev==='warn-high');
      }else if(target==='medium'){
        match=(itemSev==='medium'||itemSev==='med'||itemSev==='warning'||itemSev==='warn'||itemSev==='p1'||itemSev==='p2');
      }else if(target==='low'||target==='info'){
        match=(itemSev==='low'||itemSev==='info'||itemSev==='p3'||itemSev==='notice');
      }
      if(match){
        item.style.display='';
        visibleCount++;
    ...
    let emptyState=document.getElementById('findingsEmptyFilterState');
    if(visibleCount===0){
      if(!emptyState){
        emptyState=document.createElement('div');
        emptyState.id='findingsEmptyFilterState';
        emptyState.className='findings-empty-filter-state';
    ...
    if(countEl){
      if(target==='all'){
        countEl.textContent=`${data.findings.length} ${D[lang].issues}${counts.rawTotal>data.findings.length?` (${counts.rawTotal} ${isTr?'sayfa örneği':'page instances'})`:''}`;
      }else{
        const rawLabel=btn.textContent.replace(/\s*\(\d+\)/,'').trim();
