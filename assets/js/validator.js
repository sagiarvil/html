...
function openSaasRemediationModal(data){
  ...
  modal.innerHTML=`<div class="saas-modal-dialog">
    ...
    <div class="saas-modal-mission-banner" style="background:rgba(56,189,248,0.08);border:1px solid rgba(56,189,248,0.25);border-radius:12px;padding:12px 16px;font-size:12.5px;line-height:1.6;color:#e2e8f0;margin:4px 0 10px;">
    ...
    <div id="mtab-delivery" class="saas-modal-view" style="display:none;">
      ...
      <div class="saas-modal-guarantees" style="gap:12px;">
        <div>🛡️ <strong>${isTr?'Hizmet Sınırı:':'Scope Boundary:'}</strong> ${isTr?'HTML&HTML bir analiz ve reçete platformudur. Müşteri sitelerinde doğrudan düzeltme çalışması veya canlı proxy işletmeciliği yapmayız. Üretilen 24 dosyalık reçeteler müşterinin kendi yazılımcısına teslim edilir.':'HTML&HTML provides automated diagnostics and recipes. We do not perform manual code fixes on customer servers. The client delivers the report to their own software developers.'}</div>
        <div>⚡ <strong>${isTr?'Anında ZIP İndirme:':'Instant ZIP Delivery:'}</strong> ${isTr?'Ödeme tamamlandığı an 24 dosyalık tam mühendislik paketi (ZIP), C2PA JSON-LD, AST optimizasyon kodları ve n8n iş akışları anında indirilebilir.':'Upon payment, the complete 24-file engineering pack (ZIP), C2PA JSON-LD, AST purge templates, and n8n workflows are generated for instant download.'}</div>
...
function openBoardMemoModal(data){
  ...
  modal.innerHTML=`
    <div class="board-memo-paper">
      ...
      <p class="board-memo-p">${isTr?`Şirketimizin ana alan adı (<strong>${safe(cDomain)}</strong>) üzerinde yürütülen yapay zeka arama motorları (Perplexity, SearchGPT, Claude, Gemini) denetiminde; 14KB AST bütçe aşımı ve şema eksikliği nedeniyle aylık nitelikli kurumsal müşteri / B2B satış fırsatlarının doğrudan rakiplere yönlendirildiği tespit edilmiştir.`:`Audited primary domain (<strong>${safe(cDomain)}</strong>) reveals critical token bloat exceeding 14KB AST budget window, resulting in qualified B2B enterprise leads leaking to competitors monthly.`}</p>
      ...
      <ul style="font-size:12.5px;line-height:1.6;color:#334155;padding-left:18px;margin:0 0 14px;">
        ...
        <li><strong>${isTr?'Knowledge Vault @graph:':'Knowledge Vault @graph:'}</strong> ${isTr?'Wikidata QID ve Corporation şeması doğrudan &lt;head&gt; içine eklenerek marka teyit edilir.':'Wikidata sameAs QID and Corporation triples anchor brand in Google &amp; Perplexity.'}</li>
...
const workerSaasHtml=`<div class="saas-edge-dashboard"><div class="saas-edge-head"><div><span class="saas-status-badge"><span class="saas-status-pulse"></span> ${isTr?'● ÖRNEK EDGE WORKER MİMARİSİ (YAZILIMCINIZA TESLİM EDİN)':'● PRODUCTION EDGE WORKER TEMPLATE (FOR YOUR DEVELOPERS)'}</span><h4 style="margin:8px 0 2px;font-size:16px;color:#38bdf8;">${isTr?'Cloudflare Worker Edge Mimari Şablonu':'Cloudflare Worker Edge Architecture Template'}</h4><p style="margin:0;font-size:12px;color:#94a3b8;">${isTr?'Yazılımcınızın sitenize ekleyebileceği veya Cloudflare Worker olarak kurabileceği 14KB bütçe ve JSON-LD şablonu:':'Production-ready edge worker templates your development team can deploy to stream sub-14KB HTML and JSON-LD graphs:'}</p></div></div>
...
<div style="margin-top:8px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;padding:12px 14px;background:rgba(2,132,199,0.1);border:1px solid rgba(56,189,248,0.3);border-radius:12px;"><div style="font-size:12px;color:#e0f2fe;">🔒 <strong>${isTr?'Mühendislik Kod Şablonu:':'Engineering Code Template:'}</strong> ${isTr?'Yazılımcınıza teslim edebileceğiniz 24 dosyalı hazır mühendislik çözüm paketi.':'24-file ready-to-deploy code and recipe package for your in-house engineering team.'}</div><a href="/checkout?plan=pro&amp;domain=${encodeURIComponent(cleanDomainSafe)}&amp;scan=${encodeURIComponent(data.scanId)}" class="btn-stop-loss" style="padding:10px 20px;font-size:13px;">${isTr?'24 Dosyalık Çözüm Paketini İndir — $99 →':'Download 24-File Resolution Pack — $99 →'}</a></div></div>`;
...
> Verified Ground-Truth Entity Index for AI Engines...</div>
  ...
  <div style="margin-top:8px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;padding:12px 14px;background:rgba(168,85,247,0.1);border:1px solid rgba(168,85,247,0.3);border-radius:12px;"><div style="font-size:12px;color:#f3e8ff;">⚡ <strong>/llms.txt</strong> ${isTr?'Yazılımcınızın sitenize yükleyeceği doğrulanmış Spec-v2 şablonu.':'Spec-v2 validated markdown template ready for your developers to deploy.'}</div><a href="/checkout?plan=pro&amp;domain=${encodeURIComponent(cleanDomainSafe)}&amp;scan=${encodeURIComponent(data.scanId)}" class="btn-stop-loss" style="background:linear-gradient(135deg,#9333ea 0%,#2563eb 100%);box-shadow:0 8px 20px -4px rgba(147,51,234,0.4);padding:10px 20px;font-size:13px;">${isTr?'24 Dosyalık Çözüm Paketini İndir — $99 →':'Download 24-File Resolution Pack — $99 →'}</a></div>
</div>`;
