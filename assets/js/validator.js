...
function calcArr(qVal, dVal){const lossFraction=Math.max(0.12, (100 - overall)/100);const annualDeflection=Math.round(qVal * 12 * 0.04 * lossFraction * (dVal * 0.08));return annualDeflection}
let curQueries=50000, curDeal=1500;
const initialArr=calcArr(curQueries, curDeal);
simDeck.innerHTML=`<div class="executive-deck-head"><div><span class="executive-deck-badge">🤖 ${isTr?'GİZLİ // CANLI LLM ARAMA VE HALÜSİNASYON SİMÜLASYONU':'CONFIDENTIAL // EMPIRICAL LLM GROUNDING PROBE'}</span><h3 class="executive-deck-title">${isTr?'Yapay Zeka Modelleri Sitenizi Nasıl Görüyor? (Canlı Simülasyon)':'How Foundation AI Engines Retrieve Your Domain'}</h3><p class="executive-deck-desc">${isTr?'Perplexity, ChatGPT, Claude ve Gemini modellerinin sitenizi tararken karşılaştığı deterministik engeller ve alıntı düşme mekanizmaları:':'Empirical failure modes and mechanistic citation demotions observed across production AI search crawlers:'}</p></div></div>
<div class="simulation-arr-box"><div class="arr-metric-wrap"><span class="arr-metric-label">${isTr?'Tahmini Yıllık AI Gelir / Pipeline Kaybı (ARR at Risk)':'Estimated Annual Revenue Deflection (ARR at Risk)'}</span><strong class="arr-metric-val" id="arrMetricVal">$${initialArr.toLocaleString('en-US')} USD / ${isTr?'yıl':'yr'}</strong></div><p class="arr-metric-context">${isTr?'Sektörünüzdeki satın alma niyetli sorgularda sitenizin cevap kümesine alınmaması sebebiyle arama trafiğinin %65\'i doğrudan rakiplerinize ve üçüncü taraf dizinlere yönlenmektedir.':'Critical architectural blockers cause an estimated 65% of category commercial queries to deflect to competitors and aggregators.'}</p>
<div class="arr-calculator-controls"><div class="arr-calc-col"><div class="arr-calc-label"><span>${isTr?'Aylık Sektörel AI Arama Hacmi':'Monthly Category AI Queries'}:</span> <b id="lblQueries">50,000</b></div><input type="range" class="arr-calc-slider" id="sliderQueries" min="10000" max="500000" step="10000" value="50000"></div><div class="arr-calc-col"><div class="arr-calc-label"><span>${isTr?'Ortalama Müşteri / Sipariş Değeri':'Average Customer Contract (ACV)'}:</span> <b id="lblDeal">$1,500</b></div><input type="range" class="arr-calc-slider" id="sliderDeal" min="200" max="10000" step="100" value="1500"></div></div></div>
...
const PROBE_DATA={pplx:{name:'🟣 Perplexity Pro (Sonar-Large)',prompt:`"${safe(data.domain)} ${isTr?'kurumsal hizmetleri, fiyatlandırma ve yetkinlikleri':'enterprise solutions, pricing & architecture'}"`,rawStatus:isTr?'Alıntı Reddedildi':'Zero-Citation',rawClass:'sim-tag-red',rawReason:isTr?'<strong>Kök Neden: ENTITY-VAULT-001</strong> (Wikidata QID / Knowledge Vault bağlantısı yok). Model, markayı doğrulanmış egemen varlık saymadığı için kaynak olarak rakipleri seçti.':'<strong>Root Cause: ENTITY-VAULT-001</strong> (Missing Wikidata QID). Perplexity cannot triangulate brand into persistent Knowledge Vault, defaulting to competitors.',fixedReason:isTr?'<strong>Doğrulandı:</strong> Wikidata QID ve Crunchbase sameAs JSON-LD entegrasyonu sayesinde Perplexity Sonar markayı birincil kaynak olarak seçti. [Alıntı 1: https://'+safe(data.domain)+'/]':'<strong>Ground Truth Verified:</strong> With Wikidata sameAs QID and verified Knowledge Graph triples, Perplexity pins domain as primary citation. [Citation 1: https://'+safe(data.domain)+'/]'},sgpt:{name:'🟢 OpenAI SearchGPT & Operator',prompt:`"${safe(data.domain)} ${isTr?'teknik mimari şartname ve API uç noktaları':'technical specification & API endpoints'}"`,rawStatus:isTr?'14KB Erken Kesilme':'14KB Truncated',rawClass:'sim-tag-amber',rawReason:isTr?'<strong>Kök Neden: TOKEN-BLOAT-001 & RAG-CHUNK-001</strong>. Sayfa 14KB KV-Cache sınırını aştığı için DOM erken kesildi; ürün detayları model hafızasına yazılmadı.':'<strong>Root Cause: TOKEN-BLOAT-001 & RAG-CHUNK-001</strong>. HTML payload exceeds 14KB sub-chunk budget, severing entity vectors in RAG memory.',fixedReason:isTr?'<strong>KV-Cache Optimize Edildi:</strong> Cloudflare AST purge middleware devreye girdi; 14KB altı temiz veri SearchGPT Operator tarafından eksiksiz indekslendi.':'<strong>KV-Cache Optimized:</strong> Cloudflare edge purge reduces DOM payload below 14KB; Operator reads semantic data-chunk-id with 100% fidelity.'},claude:{name:'🟡 Anthropic Claude 3.5 Sonnet',prompt:`"${safe(data.domain)} ${isTr?'sektör benchmarkları ve güvenilirlik kanıtı':'verified industry benchmarks & citations'}"`,rawStatus:isTr?'DPO Ceza Filtresi':'DPO Demoted',rawClass:'sim-tag-amber',rawReason:isTr?'<strong>Kök Neden: CORROBORATION-RING-001 & DPO-RLAIF-001</strong>. Tek kaynaklı pazarlama dili ve metodolojik olmayan iddialar modelin Bayesçi güvenlik filtresine takıldı.':'<strong>Root Cause: CORROBORATION-RING-001 & DPO-RLAIF-001</strong>. Unmethodologized claims trigger preference penalties, demoting passage from synthesized answer.',fixedReason:isTr?'<strong>DPO Hizalaması Sağlandı:</strong> Bağımsız DOI/RFC ve üçüncü taraf benchmark korroborasyonu ile Claude Bayesçi güven filtresinden en yüksek güven puanını aldı.':'<strong>DPO Aligned:</strong> Neutral numerical formulation with independent benchmark corroboration elevates brand into Claude\'s synthesized response.'},gemini:{name:'🔵 Google AI Overviews & Gemini',prompt:`"${safe(data.domain)} ${isTr?'kurumsal varlık ve organizasyon kimliği':'corporate entity & organization profile'}"`,rawStatus:isTr?'Yüzeysel Ontoloji':'Shallow Graph',rawClass:'sim-tag-blue',rawReason:isTr?'<strong>Kök Neden: ONTOLOGY-SUPERCLASS-001</strong>. JSON-LD şeması derin hiyerarşik miras (Thing -> Organization -> Corporation) taşımadığı için Bilgi Grafiği bağı kurulamadı.':'<strong>Root Cause: ONTOLOGY-SUPERCLASS-001</strong>. Shallow JSON-LD ontology prevents entity from anchoring into persistent enterprise Knowledge Graph nodes.',fixedReason:isTr?'<strong>Bilgi Grafiği Eşleşti:</strong> Derin ontolojik JSON-LD şeması (Corporation -> knowsAbout -> sameAs) Google AI Overviews kutusunda doğrudan panel açtı.':'<strong>Knowledge Graph Anchored:</strong> Full superclass ontology links domain directly into Google\'s Knowledge Vault for guaranteed AI Overviews anchoring.'}};
...
const lockPaneHtml=(preId,content,filename,dlId)=>`<div class="code-action-bar"><span ${preId==='code-worker-pre'?'id="edgeFileTitle"':''}>${safe(filename)}</span><div><button type="button" class="btn-download-blob" id="${dlId}" style="margin-right:6px;">💾 ${isTr?'İndir — $99':'Download — $99'}</button><button type="button" class="btn-copy-code" data-target="${preId}">${isTr?'Kopyala':'Copy'}</button></div></div><div class="locked-fix"><div class="locked-fix-blurred"><pre id="${preId}" class="code-snippet-pre">${safe(content)}</pre></div><div class="locked-fix-overlay"><span>🔒 ${isTr?'$99 Onarım Seti ile Kilidi Açılır (Tam Üretim Kodu, Edge Middleware ve N8N Otomasyonu)':'Unlock with $99 Repair Kit (Full Production Code, Edge Middleware & N8N Automation)'}</span><a href="/checkout?plan=pro&amp;domain=${encodeURIComponent(data.domain)}&amp;scan=${encodeURIComponent(data.scanId)}" class="locked-fix-btn">${isTr?'Onarım Setini İndir — $99 →':'Unlock Repair Kit — $99 →'}</a></div></div>`;

remConsole.innerHTML=`<div class="executive-deck-head"><div><span class="executive-deck-badge">⚡ ${isTr?'KURUMSAL ONARIM VE ENTEGRASYON KONSOLU':'TURNKEY REMEDIATION ARTIFACTS'}</span><h3 class="executive-deck-title">${isTr?'Tek Tıkla Uygulanabilir Kod ve Entegrasyon Şablonları':'Production-Ready Code & Automation Templates'}</h3><p class="executive-deck-desc">${isTr?'Mühendislik ekibinizin veya AI coding ajanınızın (Cursor, Claude Code, Cline) doğrudan kullanabileceği üretim seviyesinde kodlar ($99 Onarım Seti ile tam erişim):':'Copy-pasteable production implementations for your engineering team or AI coding agent (Full access with $99 Repair Kit):'}</p></div></div>
<div class="n8n-dag-container"><div class="n8n-dag-title-row"><div class="n8n-dag-title"><span>⚡ ${isTr?'n8n Kendi Kendini Onaran (Self-Healing) DAG Akışı':'n8n Self-Healing DAG Orchestration Flow'}</span></div><div class="n8n-dag-actions"><button type="button" class="btn-run-dag" id="btnRunDag">▶️ ${isTr?'Akışı Test Et':'Run Test Pipeline'}</button><button type="button" class="btn-download-blob" id="btnDlN8nJson">💾 ${isTr?'n8n-workflow.json İndir — $99':'Download n8n-workflow.json — $99'}</button><button type="button" class="btn-copy-code" data-target="code-n8n-pre">${isTr?'Kopyala':'Copy'}</button></div></div>
