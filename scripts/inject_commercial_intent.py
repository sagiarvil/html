...
'tr':{
 ...
 'scan':'/tr/site-tarama/','fix':'/tr/fix-mandate/','glossary':'/tr/sozluk/','glossary_label':'AI Arama Sözlüğünü Aç',
 'stages':[
  ...
  ('02','YAPAY ZEKA SİTENİZİ ANLAYABİLSİN','GEO, AEO, LLMO, entity graph, schema ve answer extractability makine için belirsizliği azaltır.'),
 ...
 'terms':[('llms.txt','/tr/rehberler/llms-txt/'),('GEO','/tr/geo-optimizasyon/'),('AEO','/tr/aeo-answer-engine/'),('LLMO','/tr/llmo-optimizasyon/'),('AAO','/tr/aao-ajent-optimizasyon/'),('RAG','/tr/rag-optimizasyon/'),('E-E-A-T','/tr/e-e-a-t-guven-sinyalleri/'),('Sitemap','/tr/sozluk/#sitemap')],
...
'en':{
 ...
 'scan':'/en/website-scanner/','fix':'/en/fix-mandate/','glossary':'/en/glossary/','glossary_label':'Open AI Search Glossary',
 'stages':[
 ...
 'terms':[('llms.txt','/en/guides/llms-txt/'),('GEO','/en/geo-optimization/'),('AEO','/en/aeo-answer-engine-optimization/'),('LLMO','/en/llmo-optimization/'),('AAO','/en/aao-agent-optimization/'),('RAG','/en/rag-readiness/'),('E-E-A-T','/en/e-e-a-t-trust-signals/'),('Sitemap','/en/glossary/#sitemap')],
...
INTENT={
'tr':{
 'llms-txt-validator':'llms.txt merakı gerçek; ancak fırsatı doğru satın. Google Search bunu yapay zeka görünürlüğü için kullanmıyor. HTML&HTML llms.txt’yi gelişen bir makine-okunabilir bilgi yüzeyi olarak doğrular ve onu sitemap, robots.txt, crawlable HTML ve iç linklerin yerine koymaz.',
 ...
 'schema-validator':'Yapay zeka sistemlerinin kim olduğunuzu, ne sunduğunuzu ve sayfalarınızın nasıl bağlandığını daha az belirsizlikle çözebilmesi için entity graph ve yapısal veri bütünlüğini doğrulayın.',
 ...
 'teknik-seo-kontrol':'Klasik teknik SEO artık yalnız mavi linkler için değildir. Google’ın üretken yapay zeka özellikleri de Arama dizinine dayanır; taranabilirlik, indexability, canonical ve iç linkler hâlâ temeldir.',
...
def route_lead(rel,lang,default):
    for k,v in INTENT[lang].items():
        if f'/{k}/' in f'/{rel}': return v
    return default

def section_html(rel,lang,evidence):
    c=COPY[lang]
    stages=''.join(f'<article><b>{n}</b><h3>{t}</h3><p>{d}</p></article>' for n,t,d in c['stages'])
    terms=''.join(f'<a href="{u}">{t}</a>' for t,u in c['terms'])
    ev=''
    if evidence:
        cards=''.join(f'<article><b>{n}</b><p>{d}</p><a href="{u}" target="_blank" rel="noopener noreferrer'>{"Kaynağı aç" if lang=="tr" else "Open source"} ↗</a></article>' for n,d,u in c['sources'])
        ev=f'<div class="ai-market-evidence"><h3>{c["evidence_title"]}</h3><div>{cards}</div></div>'
    return (
      ...
      f'<p class="ai-opportunity-guard">{c["guard"]}</p>{ev}</div></section>'
    )
...
def inject(path):
    ...
    if '<footer' not in text: return False
    lang=detect_lang(text)
    text=re.sub(r'<section class="ai-opportunity" data-commercial-intent="static">.*?</section>\s*(?=<footer)', '', text, flags=re.S)
    if '/assets/css/commercial-intent.css' not in text:
        text=text.replace('</head>','<link rel="stylesheet" href="/assets/css/commercial-intent.css?v=2" data-commercial-intent-css="static">\n</head>',1)
    else:
    ...
    nav_link = '<a href="/tr/sozluk/">Sözlük</a>' if lang=='tr' else '<a href="/en/glossary/">Glossary</a>'
    if nav_link not in text:
        text=re.sub(r'(<header class="topbar".*?<nav>)(.*?)(</nav>)', lambda m:m.group(1)+m.group(2)+nav_link+m.group(3), text, count=1, flags=re.S)
