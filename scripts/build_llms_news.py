...
def format_paragraphs(text):
    ...
    for chunk in raw_chunks:
        # Split chunk into sentences for digestible editorial pacing
        sentences = re.split(r'(?<=[.!?])\s+(?=[A-ZÇĞİÖŞÜ0-9"\'])', chunk)
        if len(sentences) <= 2:
            ...
            for s in sentences:
                cur.append(s)
                if len(cur) >= 2:
                    paras.append(f'<p>{esc(" ".join(cur))}</p>')
                    cur = []
            if cur:
                paras.append(f'<p>{esc(" ".join(cur))}</p>')
    return ''.join(paras)
...
def write_article(item):
    ...
    for lang in ('tr', 'en'):
        ...
        summary_html = format_paragraphs(item['summary'][lang])
        why_html = format_paragraphs(item['whyItMatters'][lang])
        tech_html = format_paragraphs(item['technicalImpact'][lang])
