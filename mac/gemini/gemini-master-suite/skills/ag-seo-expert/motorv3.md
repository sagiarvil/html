...
**Entegrasyon:** Mandate v6.0 + 18 Engine V2/V3 + n8n Principles + 30-Year Veteran Engineering + Silicon Valley Reverse Engineering  
**Belge Kodu:** `MANDATE-SUPER-2026-V3`  
**Sürüm:** 3.6.0 Enterprise Exclusive Master Mandate  
...
## 📊 BÖLÜM 0: VERİ ANALİZİ VE SİSTEM GÜNCELLEME RAPORU

### 0.1 Dosya ve Mimari Karşılaştırma Matrisi (Delta Analizi)

`$HOME/.gemini\config\skills\ag-seo-expert` çalışma alanında gerçekleştirilen veri analizi neticesinde eski sürüm (`Super-Seo-Motor-V3.txt`) ile yeni gelen güncelleme paketi (`Super-seo-Mandate Motor V3.txt`), mevcut test araçları (`tools/`), yardımcı betikler (`scripts/`) ve taşınabilir denetim şartnamesi (`seotest.md`) çapraz olarak incelenmiştir.

| Boyut / Kriter | Eski Sürüm (V3.0 Base) | Yeni Sürüm (V3.6.0 Master Mandate) | Delta Değişimi & Katma Değer |
|---|---|---|---|
| **Dil & Terminoloji** | İngilizce ağırlıklı temel teknik metin | %100 Türkçe, Kurumsal Silikon Vadisi ve Unix Mühendislik Standartları | Yerel ekipler ve LLM ajanları için sıfır kayıp anlamsal netlik |
| **Toplam Satır & Hacim** | 1011 Satır (~74 KB) | 1370 Satır (~86 KB) | +359 Satır, +12 KB yeni derin istihbarat ve kod reçetesi |
| **Mimarisi Katmanları** | 4 Katmanlı Topoloji | 4 Katmanlı Topoloji + 3 Düzlemli İstihbarat (Plane A, B, C) | Gözlemlenen AI İstihbaratı ve Karar Düzlemi eklendi |
| **Tarama Hattı (Pipeline)** | Standart 6 aşama | 8 Aşamalı Deterministik Tarama Hattı (Faz 0 - Faz 7) | DNS-over-HTTPS (DoH), SSRF Kalesi ve 1MB Stream Koruması eklendi |
| **Kök-Onarım Şartnamesi** | 12 Alanlı Temel Reçete | 24 Alanlı Kök-Onarım Şartnamesi (Remediation Blueprint) | Stop-Condition Guard, SLA matrisi ve 30 Günlük Delta Tracker eklendi |
| **Lens Mimarisi** | 5 Temel Alan | 360 Derece 7 Temel Hazırlık Lensi (SEO, GEO, AEO, LLMO, AAO-Pro, RAG, E-E-A-T) | Ağırlıklı Lens Sentez Formülü tam tanımlandı |
| **Derin İstihbarat Denetimleri** | Belirtilmemiş | 13 Derin İstihbarat Denetimi (Varlık grafı, render eşitliği vb.) | Arama niyeti ve kannibalizasyon algoritmaları eklendi |
| **Kara Kutu Risk Katmanı** | Belirtilmemiş | 6 İleri Seviye Kara Kutu Risk Analizi | Sorgu yayılımı, alıntı oynaklığı ve bot ayrışması eklendi |
| **CI/CD Kalite Kapıları** | G0–G9 (10 Kapı) | G0–G15 (16 Biçimsel Kalite Kapısı) | Knowledge Vault, AST 14KB, n8n DAG ve Otonom Ajan kapıları eklendi |
| **Yerel Araç Entegrasyonu** | Sadece teorik yönergeler | 11 İki Dilli Araç (`araclar/`), 4 Çift Dilli Script (`scripts/`) | Hem Node.js hem Python ile tam deterministik yerel icra motoru |

---
İÇİNDEKİLER TABLOSU
Yönetici Mimarisi & $5M Kurumsal Paradigma
...
SSOT Registry & Kanonik Gerçeklik Anayasası
2.1 Kanonik Gerçeklik Sözleşmesi (Single Source of Truth)
2.2 TypeScript Tip Tanımları & Sözleşme Arayüzü
n8n Operasyonel Prensipleri & Dayanıklı Olay Döngüleri
3.1 6 Düğümlü İcra Zinciri (DAG)
3.2 Dead-Letter Queue (DLQ) & Hata İzolasyonu
8 Aşamalı Deterministik Tarama Hattı (Scanning Pipeline)
Faz 0: Girdi Normalizasyonu & SSRF Kalesi (DoH Doğrulaması)
...
Deterministik Reçete & Onarım Motoru (P0–P3 Kök Neden Sözleşmesi)
5.1 Onarımın 10 Anayasal Kuralı (Rules 0.1 – 0.10)
5.2 Yanlış Pozitif Doğrulama Kapıları (Kapı A – Kapı E)
5.3 Deterministik Problem Kümeleri (Root-Cause Clusters)
5.4 Matematiksel Önceliklendirme Matrisi (P0, P1, P2, P3)
5.5 24 Alanlı Kök-Onarım Şartnamesi (Remediation Blueprint)
5.6 Uygulama Durdurma Koruması (Stop Condition Guard)
5.7 30 Günlük Yeniden Tarama & Fark Motoru (Delta Tracker)
360 Derece 7 Temel Hazırlık Lensi (Readiness Lenses)
6.1 Lens 1: SEO (Search Engine Optimization)
6.2 Lens 2: GEO (Generative Engine Optimization)
6.3 Lens 3: AEO (Answer Engine Optimization)
6.4 Lens 4: LLMO (Large Language Model Optimization)
6.5 Lens 5: AAO-Pro (Autonomous Agent Optimization)
6.6 Lens 6: RAG (Retrieval-Augmented Generation)
6.7 Lens 7: E-E-A-T (Experience, Expertise, Authoritativeness, Trustworthiness)
6.8 Lens Sentez Formülü & Sınır Açıklamaları
13 Derin İstihbarat Denetimi (Intelligence Audits)
6 İleri Seviye Kara Kutu Risk Analizi (Advanced Black-Box Risk Layer)
18 Motorlu Engine V3.0 Deterministik Formülleri & Kural Matrisi
Kapsamlı Teşhis Kataloğu & Üretime Hazır Kod Reçeteleri
Vektör 1: Temel Teknik & Yapısal SEO (ENG-04, ENG-12)
Vektör 2: GEO & 14KB AST Bütçesi (ENG-01, ENG-02, ENG-05)
Vektör 3: AEO & Sesli Arama Optimizasyonu (ENG-06)
Vektör 4: LLMO & Çok Katmanlı Makine Yüzeyleri (ENG-07)
Vektör 5: AAO-Pro & Başsız Ticaret / Ajan Protokolü (ENG-13)
Vektör 6: RAG & Yoğun Bilgi Erişimi (ENG-09, ENG-10)
Vektör 7: E-E-A-T & Knowledge Vault Konsensüs Kilidi (ENG-08, ENG-14, ENG-15)
Vektör 8: Savunma, Zamansal Kararlılık & Güvenlik Sertleştirmesi (ENG-16, ENG-17, ENG-18)
Katı Mühendislik Uygulama Sırası & Sprint Mimarisi (Sprint 0 – Sprint 4)
CI/CD Kalite Kapıları (G0–G15 Biçimsel Doğrulama)
Edge CDN & Tokenomik Mimari
Gerçek Zamanlı Keşif & Çok Merkezli IndexNow Dağıtımı
30 Dosyalık Deterministik Teslimat Paketi ($99 Modeli)
15.1 İkili STORE CRC32 ZIP Derleme Mimarisi
15.2 Sıfır Bilgi Yetkilendirme & Webhook Doğrulaması
Master LLM Yürütücü Sistem Promptu (Enjeksiyona Hazır)
İmza, Anayasal Hüküm & Yürürlük Teyidi
## 1. YÖNETİCİ MİMARİSİ & $5M KURUMSAL PARADİGMA
1.1 Kurumsal Uçurum: Klasik SEO Ajanslarının LLM Çağındaki İflası
Geleneksel SEO ajansları (Londra, New York, San Francisco, İstanbul) şirketlerden yıllık 
50
,
000
i
l
a
50,000ila200,000+ arasında bütçeler talep ederek anahtar kelime takibi, manuel backlink inşası ve yüzeysel meta etiket düzenlemeleri satmaktadır. 2026 yılı itibarıyla bu yöntemler teknik olarak iflas etmiştir.
...
Knowledge Vault & Wikidata Mutabakatı: LLM'ler parametrik bilgi depolarına başvurur. Bir varlık doğrulanmış Wikidata QID veya Google Knowledge Graph MID üçlüsüne sahip değilse, halüsinasyon bastırma filtresine takılır ve alıntılanmaz.
...
## 2. SSOT REGISTRY & KANONİK GERÇEKLİK ANAYASASI
2.1 Kanonik Gerçeklik Sözleşmesi (Single Source of Truth)
Dosya Konumu: src/seo/registry.ts
Anayasal Kural: Hiçbir HTML sayfası, JSON-LD şeması, sitemap girdisi veya LLM alt grafı SEO_REGISTRY içinde açık bir karşılığı olmadan var olamaz. Kayıt defteri tek kanonik gerçektir. Kayıtta yer almayan herhangi bir rota, CI derleme kapısında derhal reddedilir.
...
export interface SeoPageRecord {
  ...
  readonly modifiedAt: string;       // GERÇEK güncelleme — Asla sahte tarih yok
...
01 · TRIGGER	n8n-nodes-base.scheduleTrigger	Cron 0 3 * * * (Her gün 03:00 UTC), HMAC-SHA256 imzalı tetikleme, 3x üstel geri çekilme.	Girdi: { cron: "0 3 * * *" }
Çıktı: { eventId, timestamp, targetQueue }
02 · PROBE	n8n-nodes-base.httpRequest	/llms.txt ve /llms-full.txt uç noktalarını 8000ms sert zaman aşımıyla yoklar.	Girdi: { target: "domain.com/llms.txt" }
Çıktı: { status: 200, latencyMs: 24, bytes: 4096 }
03 · INGEST	n8n-nodes-base.httpRequest	PerplexityBot, GPTBot, ClaudeBot User-Agent'larıyla eşzamanlı HTTP/3 0-RTT yoklaması.	Girdi: { botAgents: ["GPTBot", "ClaudeBot"] }
Çıktı: { rawAst, headers, cfCacheStatus }
04 · AUDIT	n8n-nodes-base.code	Sub-14KB AST ilk paket bütçesi, data-chunk-id sınırları ve Wikidata QID üçlülerini denetler.	Girdi: { rawAst, byteLength }
Çıktı: { byteBudgetPass: true, astTokens: 3120 }
05 · TRIAGE	n8n-nodes-base.if	Bayesyen kayma kapısı: Genel skor < 80/100 veya gerileme tespit edilirse Onarım Dalına yönlendirir.	Girdi: { score: 74, threshold: 80 }
Çıktı: { branch: "INCIDENT_TRIAGE_DLQ" }
06 · AUTO-HEAL	n8n-nodes-base.httpRequest	Cloudflare Edge Cache Purge API'sini çağırır, bayat etiketleri temizler, Slack/PagerDuty alarmı açar.	Girdi: { purgeTags: ["semantic-ast", "llms-txt"] }
Çıktı: { purged: true, edgeVerified: true }
3.2 Dead-Letter Queue (DLQ) & Hata İzolasyonu
...
GET /robots.txt: Bot kuralları ayrıştırılır (Googlebot, OAI-SearchBot, Claude-SearchBot, PerplexityBot, GPTBot, ClaudeBot, Google-Extended).
GET /sitemap.xml: İlk 100 dahili <loc> URL'si çıkarılır.
GET /llms.txt: v2 spesifikasyonu doğrulanır (# Domain H1, > Summary alıntısı, Markdown bağlantıları).
GET /.well-known/agent-card.json: A2A otonom ajan keşif kartı şeması doğrulanır.
GET /openapi.json: OpenAPI 3.0/3.1 makine spesifikasyonu doğrulanır.
GET /mcp: Model Context Protocol JSON-RPC araç uç noktası test edilir.
GET Wikidata SPARQL API: https://www.wikidata.org/w/api.php üzerinden marka QID varlığı sorgulanır.
GET Common Crawl CDX: index.commoncrawl.org üzerinden alan adının LLM ön-eğitim korpusundaki varlığı sorgulanır.
Faz 3: Sığ Tarama Alt Grafı (Maksimum 50 Sayfa İzolasyonu)
...
Rule 0.3 (Otonom Kod Teslimi): Öneriler değil; kopyala-yapıştır çalışır tam kod blokları (HTML, JS, Cloudflare Worker, Nginx, JSON-LD) sunulur.
...
×
Confidence
PriorityScore=SeverityWeight×ScopeReach×Confidence

Öncelik	Puan Eşiği	Etki Açıklaması	SLA / Çözüm Penceresi
P0	
≥
90
≥90	HTTP 5xx hataları, kazara noindex, güvensiz formlar, ciddi SSR yetersizliği.	0 – 48 Saat
P1	
75
−
89
75−89	Eksik kanonik, 14KB AST aşımı, geçersiz JSON-LD, engellenen arama botları.	Gün 3 – Gün 7
P2	
50
−
74
50−74	Eksik /llms.txt, Wikidata QID eksikliği, yinelenen title'lar, jenerik iç linkler.	Hafta 2 – Hafta 3
P3	
<
50
<50	Eksik A2A agent card, MCP araçları, eksik Markdown alternatifleri, DPO üslup uyumu.	Hafta 4 (Sprint Sonu)
5.5 24 Alanlı Kök-Onarım Şartnamesi (Remediation Blueprint)
...
Lens
​
 =clamp(CoreScore×0.70+IntelligenceScore×0.30) Sınır Şartı: Lens skorları kurum içi değerlendirme heuristikleridir; Google veya üçüncü taraf AI modellerinin resmi sıralama garantisi değildir.

## 7. 13 DERİN İSTİHBARAT DENETİMİ (INTELLIGENCE AUDITS)
functions/lib/intelligence-engine.ts içindeki 13 derin analiz:

Arama Niyeti ve Kannibalizasyon Denetimi (intent_cannibalization): Aynı alan adındaki sayfaların birbiriyle aynı title veya H1 kullanarak arama motoru nezdinde birbirini baltalamasını denetler.
Özgün Bilgi Değeri Sinyalleri (information_gain): Sayfanın web genelindeki kopyalardan farklılaşan birinci el veri, hesaplama araçları, karar tabloları ve tescilli metodoloji içerip içermediğini inceler.
Doğrudan Cevap Çıkarılabilirliği (answer_extractability): Sayfa metninin başlıkların hemen ardından net, bağımsız ve bağlamından koparıldığında da anlamlı kalan doğrudan cevap bloklarına sahip olup olmadığını ölçer.
Varlık Grafı Bütünlüğü (entity_graph_integrity): JSON-LD şemasında Organization, Person, Product gibi temel varlıkların tutarlı @id referanslarıyla birbiriyle örülüp örülmediğini denetler.
Güncellik Bütünlüğü (freshness_integrity): Görünür güncelleme tarihleri ile şemadaki dateModified ve sitemap'teki lastmod tarihlerinin gerçek içerik değişimine dayanıp dayanmadığını doğrular (Sahte güncellik filtresi).
Hydration / Render Eşitliği (render_parity): Ham HTTP HTML çıktısı ile tarayıcı tarafından render edilen DOM arasındaki farkı gözlemler; JS çalıştırmayan botlar için içerik kaybı olup olmadığını belirler.
LLM Bilgi Yüzeyi Denetimi (llm_knowledge_surface): /llms.txt ve bağlı Markdown kaynaklarının erişilebilirliğini, sözdizimini ve bağlantı sağlığını ölçer.
İç Bağlantı Anlamsal Uyumu (internal_link_semantic_alignment): İç bağlantıların "buraya tıklayın" gibi anlamsız metinler yerine hedef sayfanın konusunu ve varlığını tanımlayan açıklayıcı metinler (anchor text) içerip içermediğini denetler.
Yetim Sayfa Tespiti (orphan_pages): Sitemap'te yer alıp site içi hiçbir sayfadan bağlantı almayan veya menüde kaybolan sayfaları tespit eder.
Keşif Yolu Denetimi (discovery_path): robots.txt, sitemap, kanonik etiketler, llms.txt ve API yüzeylerinin tek ve tutarlı bir bilgi mimarisini işaret edip etmediğini ölçer.
IndexNow Hazırlığı (indexnow_readiness): Arama motorlarına gerçek zamanlı URL güncellemesi göndermek için gerekli IndexNow anahtar dosyasının varlığını ve yayın durumunu kontrol eder.
Yapısal @graph Tutarlılığı (structured_graph_consistency): Sayfadaki birden fazla bağımsız JSON-LD bloğunun çelişki üretmeden tek bir @graph kökü altında birleşip birleşmediğini doğrular.
Kod Tabanı SEO Yönetişimi (codebase_seo_governance): Kaynak kod seviyesindeki rota kayıt defterini, CI/CD kalite kapılarını ve meta veri üretim mekanizmalarını doğrular.
8. 6 İLERİ SEVİYE KARA KUTU RİSK ANALİZİ (ADVANCED BLACK-BOX RISK LAYER)
Harici AI sistemlerinin kapalı kutu doğasından kaynaklanan 6 kritik risk boyutu:

Sorgu Yayılımı Kapsama Riski (query_fanout_coverage): Google AI Overviews ve AI Mode tek bir kullanıcı aramasında arka planda 5-10 ilişkili alt sorgu üretir. Sayfanın bu alt dalları karşılayıp karşılayamadığını modeller.
Alıntı ve Tavsiye Oynaklığı (citation_volatility): Perplexity ve ChatGPT modellerinin zaman içinde ve sıcaklık (temperature) dalgalanmalarında markayı alıntılama sıklığındaki oynaklık riskini inceler.
Crawler Amaç ve Politika Ayrışması (crawler_policy_divergence): Arama botları (OAI-SearchBot), kullanıcı yönlendirmeli botlar (Claude-User) ve eğitim botları (GPTBot) arasındaki robots.txt ayrımının doğru kurgulanıp kurgulanmadığını denetler.
Render–Erişim Farkı (render_retrieval_gap): İstemci taraflı JavaScript (SPA / React / Vue) arkasında kalan içeriğin AI arama botları tarafından indekslenemeden atlanma riskini raporlar.
Varlık Kimliği Sapması (entity_identity_drift): Farklı kaynaklarda marka hakkında geçen çelişkili bilgilerin LLM'lerde kimlik karmaşası ve halüsinasyon yaratma riskini modeller.
Ajan İşlem Sürtünmesi (agent_action_friction): Otonom ajanların site üzerinde form doldururken, buton tıklarken veya sipariş verirken karşılaştığı erişilebilirlik ve arayüz bariyerlerini değerlendirir.
9. 18 MOTORLU ENGINE V3.0 DETERMINİSTİK FORMÜLLERİ & KURAL MATRİSİ
...
TOPLAM		129	
## 10. KAPSAMLI TEŞHİS KATALOĞU & ÜRETİME HAZIR KOD REÇETELERİ
...
Teşhis Kuralı: Crawler isteklerinde TTFB 
>
250
e
x
...
m
s
>250extms veya cf-cache-status: HIT eksikliği.
...
- [Kurumsal Kimlik ve Metodoloji](https://htmlandhtml.com/llms/core.md): Platform mimarisi, ontoloji ve kanıt standartları.
...
Teşhis Kuralı: Sayfada başlık sayısının 
<
3
<3 olması veya başlıkların teknik sorgu varyasyonlarını içermemesi.
...
<table class="w-full text-left text-sm border-collapse border border-slate-200">
  <thead>
    <tr class="bg-slate-100">
      ...
      <th class="p-2 border border-slate-200">Durum</th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="p-2 border border-slate-200 text-sky-600 font-bold">99 USD</td>
      <td class="p-2 border border-slate-200">Tek seferlik lisans. Kodlar müşterinin kendi yazılımcısına teslim edilir.</td>
    </tr>
    <tr>
      <td class="p-2 border border-slate-200">Doğrudan Kod Entegrasyonu / Danışmanlık</td>
      <td class="p-2 border border-slate-200 text-rose-600 font-bold">YOK (0 Saat)</td>
      <td class="p-2 border border-slate-200">Ajans değiliz. Müşteri sitesine dışarıdan insan müdahalesi yapılmaz.</td>
    </tr>
  </tbody>
</table>
Terminal Kabul Komutu:
...
≥
500
≥500 sunucu hatalarını ve döngüsel yönlendirmeleri çözün.
...
Çıkış Kapısı: AI User-Agent yanıtı 
<
14.336
<14.336 bayt, kanonik tutarlılık %100, sayfa başına 1 H1.
...
18 motor genel skorunun 
≥
85
/
100
≥85/100 (PASS) seviyesine ulaştığını tescil edin.
...
const INDEXNOW_CONFIG = {
  ...
  keyLocation: 'https://htmlandhtml.com/9d980417475ac56c8ad72ef2c743e1e5.txt',
  endpoints: [
...
## 15. 30 DOSYALIK DETERMINİSTİK TESLİMAT PAKETİ ($99 MODELİ)
...
## 16. MASTER LLM YÜRÜTÜCÜ SİSTEM PROMPTU (ENJEKSİYONA HAZIR)
...
- BULGU ID: [Örn: RAG-CHUNK-001]
- ETKİ DERECESİ: [KRİTİK / YÜKSEK / ORTA]
...
SSOT Registry Mimarisi (src/seo/registry.ts Kanonik Gerçeklik Anayasası)
...
unsurlarını tek bir organik mimaride birleştirmiştir.
...
∗
∗
5,000,000+kurumsalistihbaratkapasitesi∗∗TicariSatı 
s
\c
​
 Bedeli:∗∗99 (Tek seferlik lisanslı deterministik kod paketi)
Tasarım Sözleşmesi: %100 Açık Tema (#ffffff), Sıfır Koyu Alan / Sıfır Dark Bleed, WCAG AAA Kontrast
...
## 🧰 BÖLÜM 16: SUPER-SEO-MOTOR LOKAL VE CANLI ARAÇ SETİ ENTEGRASYONU

Dizindeki `araclar/` ve `scripts/` klasörleri altında yer alan tüm çift dilli (Node.js & Python) motorlar:

```
araclar/
├── _lib.js / _lib.py              → Ortak çekirdek kütüphane (HTTP bot simülasyonu, regex, JSON-LD)
...
scripts/
├── js/ & python/
│   ├── seo-folder-scanner         → Toplu proje klasörü tarayıcısı (Vektör 1-8)
│   ├── seo-checker                → Canlı tekil URL 0-RTT analiz motoru
│   ├── indexnow-pusher            → Multi-hub IndexNow push aracı
│   └── llms-generator             → llms.txt v2 ve alt kırılımları üreten motor
```

### Örnek Terminal Çalıştırma Komutları:
```bash
# 18 Motorlu Tam Denetim
node araclar/audit_engine_v3.js --url=https://target-domain.com
python araclar/python-araclar/audit_engine_v3.py --url=https://target-domain.com

# Canlı DOM ve UTF-8 Doğrulaması
node araclar/dom_utf8_full_test.js https://target-domain.com
python araclar/python-araclar/dom_utf8_full_test.py https://target-domain.com

# Yerel Proje Klasör Taraması
node scripts/js/seo-folder-scanner.js $HOME/Sites\hedef-proje
python scripts/python/seo_folder_scanner.py $HOME/Sites\hedef-proje
```

---

## 🧭 BÖLÜM 17: TAŞINABİLİR PROJE DENETİM VE DROP-IN GÜNCELLEME PROTOKOLÜ (`seotest.md`)

Herhangi bir yeni veya mevcut projeye başlandığında AI ajan / yazılım mühendisi istisnasız aşağıdaki **4 Aşamalı Süreç Döngüsünü** işletir:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│ 0. ADIM: TAŞINABİLİR YOL ÇÖZÜMLEMESİ                                        │
│    - [SEOTEST], [PROJE_KÖKÜ], [GUNCELLEME], [RAPOR], [MOTOR_KURAL] bulunur  │
│    - Değerler kullanıcıya yazılır ve onaylanır                              │
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ 1. ADIM: KURAL VE ŞARTNAMENİN OKUNMASI (SUPER-MANDATE ENGINE V3.0)          │
│    - [MOTOR_KURAL] okunur: 18 Motor, 14KB AST bütçesi, ColBERT MaxSim,      │
│      Knowledge Vault kuralları                                             │
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ 2. ADIM: CANLI SİTE ADRESİNİN KULLANICIDAN ALINMASI ([HEDEF_URL] SORGUSU)   │
│    - Canlı hedef domain alınır (Tüm alt sayfalar, sitemap, llms taranır)    │
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ 3. ADIM: CANLI SUPER-SEO-MOTOR-V3 İLE TÜM SAYFALARIN DERİNLEMESİNE TARANMASI│
│    - 8 Fazlı tarama hattı: 14KB AST, Tek H1, Schema @graph, AEO, LLMO       │
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ 4. ADIM: LOKAL PROJEDE ONARIM & ORİJİNAL KLASÖR AĞACIYLA GÜNCELLEME PAKETİ  │
│    - Düzeltilen dosyalar [PROJE_KÖKÜ]'ne göreli yollarıyla [GUNCELLEME]/    │
│      altına atılır                                                         │
