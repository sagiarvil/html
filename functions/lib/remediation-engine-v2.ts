...
export const FULL_SITE_FIX_MANDATE_PRICE_USD=99 as const;
...
function prescriptionFor(a:IntelligenceAnalysis,rank:number|null):IntelligencePrescription{
  ...
  switch(a.key){
    ...
    case 'intent_cannibalization':
      return {...base,implementation_state:'VERIFY_FIRST',root_fix:['Review colliding pages and assign one distinct primary search intent to each canonical URL.','If two pages are genuinely duplicative, consolidate content and internal authority into the stronger canonical destination; otherwise differentiate title, H1, content promise and internal anchors to reflect distinct intent.','Preserve valuable URLs with redirects only when consolidation is truly permanent.'],acceptance_tests:['No two retained same-locale priority pages have materially identical title/H1/content promise for the same primary intent.','Any consolidated URL resolves directly to its final canonical destination without redirect chains.'],regression_tests:['Canonical, sitemap, hreflang and internal links all reference the final retained route.'],rollback_guidance:['Restore the prior route/content mapping if consolidation removes a demonstrably distinct user intent or breaks existing inbound traffic paths.']};
    case 'answer_extractability':
      return {...base,implementation_state:'VERIFY_FIRST',root_fix:['Place a concise, self-contained answer immediately after the relevant question/heading on priority informational pages.','Use clear definitions, lists, tables or step sequences when they improve extraction; do not force a fixed word count or pixel position.','Ensure the extracted answer remains accurate when read without the preceding paragraph.'],acceptance_tests:['A human reviewer can identify the primary answer without reading unrelated introductory copy.','The answer block is self-contained and consistent with the page title/H1 and structured data.'],regression_tests:['Heading hierarchy remains valid and the page does not gain duplicate H1s or contradictory answers.'],rollback_guidance:['Revert the answer block if it oversimplifies, changes legal/technical meaning or duplicates another canonical page intent.']};
    case 'entity_graph_integrity':
    case 'structured_graph_consistency':
      return {...base,implementation_state:'ACTIONABLE',root_fix:['Normalize stable @id identifiers for the Organization, WebSite, WebPage and relevant Product/Service/Person entities.','Link graph nodes with explicit references instead of duplicating conflicting entity objects across pages.','Remove or correct JSON-LD properties that cannot be supported by visible/public facts.'],acceptance_tests:['All JSON-LD blocks parse successfully.','Repeated primary entities resolve to stable @id values across tested canonical pages.','Structured claims match visible/public site facts.'],regression_tests:['No schema type disappears from pages where it remains factually applicable.','Canonical and breadcrumb URLs stay aligned with public routing.'],rollback_guidance:['Restore the previous valid graph if a new relationship cannot be supported or causes parser errors.']};
    case 'freshness_integrity':
      return {...base,implementation_state:'VERIFY_FIRST',root_fix:['Align visible update dates, dateModified/datePublished and sitemap lastmod to real content changes only.','Use the actual semantic publication/update event; never advance timestamps solely to imply freshness.','Keep pages without meaningful updates unchanged rather than manufacturing activity.'],acceptance_tests:['For sampled changed pages, visible dates, structured-data dates and sitemap lastmod do not contradict each other.','No timestamp is in the future or changes without a documented content/data change.'],regression_tests:['Deploying unrelated code does not rewrite all page lastmod values.'],rollback_guidance:['Restore previous timestamps if the new value cannot be tied to a real content or data change.']};
    case 'llm_knowledge_surface':
      return {...base,implementation_state:'ACTIONABLE',root_fix:['Keep one concise root /llms.txt as a directory/manifest and link only to current, canonical, useful resources.','For deeper machine-readable content, publish scoped Markdown resources with correct Content-Type and canonical references where the product architecture supports them.','Continuously validate every linked resource; remove stale or broken machine-readable surfaces.'],acceptance_tests:['/llms.txt is reachable, structurally valid under the published proposal profile and all sampled linked URLs return usable responses.','Machine-readable content does not contradict canonical HTML.'],regression_tests:['llms.txt remains labeled as a proposal and no ranking/citation guarantee appears.'],rollback_guidance:['Remove a newly added machine-readable surface if it becomes stale, contradictory or unreachable.'],do_not_fake:[...commonDoNotFake,'Do not call llms.txt an RFC or mandatory web/AI standard.']};
    case 'internal_link_semantic_alignment':
      return {...base,implementation_state:'VERIFY_FIRST',root_fix:['Replace vague anchors on important routes with concise descriptive text that identifies the destination intent.','Ensure internal links point to the final canonical URL and reinforce the intended topical hierarchy.','Do not enforce an arbitrary semantic-similarity percentage.'],acceptance_tests:['Priority internal links use descriptive anchors and resolve to live canonical destinations.','No new broken link or avoidable redirect is introduced.'],regression_tests:['Navigation, accessibility names and user-facing copy remain understandable after anchor changes.'],rollback_guidance:['Restore prior anchor/navigation wording if the new wording harms usability or misrepresents destination intent.']};
    case 'orphan_pages':
      return {...base,implementation_state:'VERIFY_FIRST',root_fix:['For confirmed valuable orphan pages, add contextual internal links from relevant hub/category pages and include the canonical URL in the sitemap where appropriate.','For URLs not intended for discovery, remove them from indexable discovery surfaces or apply the correct index policy instead of manufacturing links.'],acceptance_tests:['Every retained indexable priority page is reachable through at least one meaningful internal path and its discovery policy is consistent.'],regression_tests:['New links do not create circular navigation clutter or duplicate-intent hubs.'],rollback_guidance:['Remove added links if the target is not intended to be indexable or the relationship is not contextually valid.']};
    case 'discovery_path':
      return {...base,implementation_state:'ACTIONABLE',root_fix:['Align robots.txt, sitemap, canonicals, internal links, llms.txt and documented API/machine-readable surfaces so they point to the same intended public information architecture.','Remove discovery references to dead, redirected or non-canonical destinations.'],acceptance_tests:['All sampled discovery surfaces resolve to intended final canonical public URLs.','Search/retrieval bot policy choices remain explicit and do not conflate search access with training controls.'],regression_tests:['SSRF/security controls and intentional noindex/private routes remain protected.'],rollback_guidance:['Revert only the affected discovery declaration if it exposes a route that should remain private or non-indexable.']};
    default:
...
function buildIntelligenceActions(intelligence:IntelligenceReport):IntelligencePrescription[]{
  ...
  return intelligence.analyses.filter(a=>a.status!=='PASS').map(a=>prescriptionFor(a,ranks.get(a.key)||null)).sort((a,b)=>(a.priority_rank??999)-(b.priority_rank??999)||a.analysis.localeCompare(b.analysis));
}
...
function appendIntelligenceMarkdown(md:string,actions:IntelligencePrescription[]):string{
  ...
  for(const a of actions){
    ...
    out+=`- **Root Fix:** ${a.root_fix.join(' ')}\n- **Acceptance:** ${a.acceptance_tests.join(' ')}\n- **Regression:** ${a.regression_tests.join(' ')}\n- **Rollback:** ${a.rollback_guidance.join(' ')}\n- **Do Not Fake:** ${a.do_not_fake.join(' ')}\n\n`;
  }
  return out;
}
...
export function generateFullSiteFixMandate(scan:ScanResult,baselineScan?:ScanResult):FullSiteFixMandateReport{
  ...
  return Object.assign(legacy,{commercial_contract:'FULL_SITE_FIX_MANDATE' as const,price_usd:FULL_SITE_FIX_MANDATE_PRICE_USD,intelligence,intelligence_actions});
}
