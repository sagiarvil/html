export const GUEST_ENTITLEMENT_VERSION=1 as const;
...
export async function createGuestEntitlement(domain:string,secret:string,ttlSeconds=3600):Promise<string>{
  if(secret.length<32)throw new Error('Guest entitlement signing secret must be at least 32 characters');
  const now=Math.floor(Date.now()/1000);const claims:GuestEntitlementClaims={v:1,product:GUEST_PRODUCT,domain:canonicalHost(domain),issuedAt:now,exp:now+Math.max(300,Math.min(ttlSeconds,86400)),nonce:crypto.randomUUID()};
...
export async function verifyGuestEntitlement(token:string,secret:string,targetDomain:string):Promise<GuestEntitlementClaims|null>{
  try{
    if(!token||secret.length<32)return null;const [body,sig,...extra]=token.split('.');if(!body||!sig||extra.length)return null;
    ...
    if(claims.v!==1||claims.product!==GUEST_PRODUCT||!claims.nonce||claims.exp<now||claims.issuedAt>now+60)return null;
