 ...
 * 3. Python Sözdizim Doğrulama (python -m py_compile)
...
// Canonical Araç Yolları
const CANONICAL_PHP = 'php';
const CANONICAL_NODE = 'node';
const USER_HOME = process.env.USERPROFILE || process.env.HOME || require('os').homedir();
const CANONICAL_PYTHON = path.join(USER_HOME, 'AppData', 'Local', 'Programs', 'Python', 'Python314', 'python3');
...
const PHP_CMD = getExecutor(CANONICAL_PHP, 'php');
const NODE_CMD = getExecutor(CANONICAL_NODE, 'node');
const PYTHON_CMD = getExecutor(CANONICAL_PYTHON, 'python');
...
const BUG_PATTERNS = [
  {
    ...
    category: 'QUALITY',
    code: 'TR-MOJIBAKE',
    msg: 'Bozuk Türkçe karakter (mojibake) kalıntısı tespit edildi (Ý, Ð, Þ, ý, ð, þ)!'
  },
  {
    regex: /header\s*\(\s*['"]Content-Type:\s*application\/json['"]\s*\)/i,
    category: 'QUALITY',
    code: 'TR-MISSING-CHARSET',
    msg: 'Content-Type: application/json başlığında charset=utf-8 eksik!'
  },
  {
    regex: /\bstrtoupper\s*\(/i,
    category: 'QUALITY',
    code: 'TR-STRTOUPPER',
    msg: 'strtoupper() kullanımı tespit edildi. Türkçe karakterler için mb_convert_case(..., MB_CASE_UPPER, "UTF-8") kullanılmalıdır.'
  },
...
function checkFile(filePath) {
  ...
  // 4. Python Syntax Kontrolü (python -m py_compile)
  if (ext === '.py') {
    try {
      execSync(`${PYTHON_CMD} -m py_compile "${filePath}"`, { stdio: ['pipe', 'pipe', 'pipe'] });
      report.stats.pass++;
    } catch (err) {
      const errOutput = (err.stderr ? err.stderr.toString('utf8') : '') || err.message;
