...
const localAttr=/(?:href|src)=["']([^"']+)["']/gi;for(const file of ['index.html','checkout.html']){const text=fs.readFileSync(path.join(root,file),'utf8');let m;while((m=localAttr.exec(text))){let t=m[1];if(!t||t.startsWith('#')||/^(https?:|mailto:|tel:|data:|javascript:)/i.test(t))continue;t=t.split('#')[0].split('?')[0];if(!t)continue;if(t==='/')t='index.html';else if(t.startsWith('/'))t=t.slice(1);else t=path.resolve(path.dirname(file),t);const resolved=path.isAbsolute(t)?t:path.join(root,t);if(!fs.existsSync(resolved))errors.push(`${file}: missing local target ${m[1]}`)}}
