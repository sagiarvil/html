...
expect(sync.includes("['delivery-pack.ts','delivery-pack.ts']")&&sync.includes("['guest-entitlement.ts','guest-entitlement.ts']"),'Firebase sync must include canonical delivery and guest entitlement engines');
...
console.log('DELIVERY CONTRACT PASS: $99 Roadmap ZIP, 11 core files, up-to-30 evidence-bound machine surfaces, signed domain+order guest entitlement, Firebase parity and fail-closed delivery verified.');
