...
function engineIds(source) {
  return [...source.matchAll(/class="ea-engine-id"[^>]*>\s*(ENG-\d{2})\s*</g)].map(m => m[1]);
}

function engineScores(source) {
  return [...source.matchAll(/class="ea-engine-score-val"[^>]*>\s*(\d+)%\s*</g)].map(m => Number(m[1]));
}

function findingCount(source) {
  return (source.match(/class="[^"]*finding-card\b/g) || []).length;
}
...
for (const [file, enPath, trPath] of cases) {
  ...
  const en = read(enRel);
  const tr = read(trRel);
  const enText = visible(en);
  const trText = visible(tr);
  ...
  assert.match(en, /enterprise-analyzer-locale\.js\?v=1/, `${enRel} runtime guard missing`);
  assert.match(tr, /enterprise-analyzer-locale\.js\?v=1/, `${trRel} runtime guard missing`);

  const enUrl = `https://htmlandhtml.com${enPath}`;
  const trUrl = `https://htmlandhtml.com${trPath}`;
  ...
  for (const leak of ['Ücretsiz', 'Çözümler', 'Haberler', 'Sözlük', 'Onarım', 'Hedef:', 'Ağırlık:', 'Tamamlandı']) {
    assert.ok(!enText.includes(leak), `${enRel} contains Turkish leakage: ${leak}`);
  }
  for (const leak of ['Free Check', 'Solutions', 'Glossary', 'Fix Mandate', 'Sample Report', 'Change Theme', 'Target:', 'Weight:', 'Attention Required']) {
    assert.ok(!trText.includes(leak), `${trRel} contains English UI leakage: ${leak}`);
  }
