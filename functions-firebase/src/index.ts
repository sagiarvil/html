...
function weightedScore(rules:Array<{ok:boolean|null;weight:number}>){
  let den=0,num=0;
  for(const rule of rules){if(rule.ok===null)continue;den+=rule.weight;if(rule.ok)num+=rule.weight}
...
function normalizePublicScanResult(result:any){
  ...
  const botCopy:Record<string,{tr:string;en:string;evidence:string}>={
    'AI-ROBOTS-OAI-SEARCHBOT':{
      tr:'OpenAI’nin arama amaçlı OAI-SearchBot erişimi robots.txt tarafından engelleniyor; OpenAI arama deneyimlerinde site keşfini etkileyebilir.',
      en:'OpenAI OAI-SearchBot access is blocked by robots.txt; this may affect site discovery in OpenAI search experiences.',
      evidence:'robots.txt decision: OAI-SearchBot path / => DISALLOW'
    },
    'AI-ROBOTS-CLAUDE-SEARCHBOT':{
      tr:'Anthropic’in arama amaçlı Claude-SearchBot erişimi robots.txt tarafından engelleniyor; ilgili arama deneyimlerinde site keşfini etkileyebilir.',
      en:'Anthropic Claude-SearchBot access is blocked by robots.txt; this may affect site discovery in related search experiences.',
      evidence:'robots.txt decision: Claude-SearchBot path / => DISALLOW'
    },
    'AI-ROBOTS-PERPLEXITYBOT':{
      tr:'PerplexityBot erişimi robots.txt tarafından engelleniyor; Perplexity’nin web keşif/tarama sürecinde sitenizin kullanılmasını etkileyebilir.',
      en:'PerplexityBot access is blocked by robots.txt; this may affect use of the site in Perplexity web discovery/crawling.',
      evidence:'robots.txt decision: PerplexityBot path / => DISALLOW'
    },
    'AI-ROBOTS-CLAUDE-USER':{
      tr:'Claude-User için kullanıcı yönlendirmeli web erişimi robots.txt tarafından engelleniyor; kullanıcı isteğine bağlı retrieval erişimini etkileyebilir.',
      en:'User-directed web access for Claude-User is blocked by robots.txt; this may affect user-requested retrieval.',
      evidence:'robots.txt decision: Claude-User path / => DISALLOW'
    }
  };
  for(const f of findings){
    const copy=botCopy[String(f?.id||'')];
    if(copy){f.impactTr=copy.tr;f.impactEn=copy.en;f.evidence=copy.evidence}
  }
  if(result.scores&&result.policies){
    ...
    for(const [key,weight] of Object.entries(weights)){
      if(typeof result.scores[key]!=='number')continue;
      numerator+=result.scores[key]*weight;denominator+=weight;
    }
    if(denominator)result.overall=Math.round(numerator/denominator);
  }
  return result;
}
...
async function safeTelemetry(input:ScanTelemetryInput){
  ...
  }catch(e:any){console.warn('scan_telemetry_unavailable',String(e?.code||e?.message||'unknown').slice(0,80))}
}
...
export const scan=onRequest({...common,timeoutSeconds:240,memory:'1GiB'},async(req,res)=>{
  ...
  try{
    ...
    await safeTelemetry({
      success:true,domain:result.domain,durationMs:Date.now()-started,pagesScanned:result.summary?.pagesScanned,linksProbed:result.summary?.linksProbed,
      findingCount:Array.isArray(result.findings)?result.findings.length:0,overall:result.overall
    });
